...
const items = [
  ...
  },
  {
    ...
    icon: categorie,
    link: "/dashboard/CategoriesManagement",
    children: [
      {
        key: "categories",
        label: "Categories",
...
const SidBar = () => {
  ...
  return (
    <div className="custom-sidebar h-full bg-[#050505]">
      {/* Logo */}
      <div className="custom-sidebar-logo flex justify-center p-4">
          ...
          <div key={item.key}>
            ...
            <div
              className={`menu-item my-4 mx-5 py-3 px-3 flex items-center cursor-pointer ${
                selectedKey === item.key ? "bg-[#EDC4C5] rounded-md" : "bg-white rounded-md hover:bg-gray-200"
              }`}
              onClick={() => {
                if (item.children) {
            ...
            >
              ...
              <Link to={item.link} className="block w-full text-black">
                {item.label}
              </Link>
            ...
            {item.children && expandedKeys.includes(item.key) && (
              <div className="overflow-hidden bg-white -my-2 mx-5 mb-4 text-black transition-all duration-300">
                {item.children.map((child) => (
                  ...
                  >
                    <Link to={child.link} className="block w-full text-black">
                      {child.label}
                    </Link>
                  </div>
                ))}
  ...
  <button className="w-full bg-white rounded-md  text-black p-3">Log Out</button>
</div>
