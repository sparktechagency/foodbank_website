...
export const InvitedWarehouseVolunteers = ({ event }) => {
  ...
  // Track loading states for "Remove Volunteer" buttons
  ...
  const handleRemoveEventGroup = async (email) => {
    // Set loading state for this specific volunteer
            ...
            {event?.warehouse?.map((ev, index) => (
              <div key={index} className="flex justify-between space-y-4">
                ...
                <button
                  ...
                  className="bg-blue-600 text-white px-3 rounded-full text-sm flex items-center justify-center"
                  disabled={removeVolunteerLoading[ev?.email]} // Disable button while loading
                >
                  {removeVolunteerLoading[ev?.email] ? ( // Show loading spinner if loading
                    <Spin size="small" />
                  ) : (
                    "Remove"
                  )}
